...
### 5.2.1 sessionStorage ключове

| Ключ | Тип / стойност | Смисъл |
|---|---|---|
| `pending_auto_upload_notice_drive` | `"1"` / липсва | Маркер, зададен точно преди клик на „Свържи с Google Drive“; ползва се от модала за напомняне за авто-качване — вижте §9.2.1 |
...
### 9.2.1 Напомняне при свързване, ако авто-качването е изключено

Проблем: по подразбиране `autoSync` е `"off"` и за двете услуги (§9.2) — само
свързването само по себе си не включва автоматично качване. Първоначално това
беше отбелязано с надпис в панела, но лесно се пропускаше, затова е заменено с
модален прозорец, който изисква изрично затваряне.

Ново състояние в `App.jsx`:

```js
const [autoUploadNotice, setAutoUploadNotice] = useState(null); // 'drive' | 'supabase' | null
```

Рендерира се модал в стила на `confirmDialog` (§13), но само с бутон „Разбрах“
(без „Откажи“) — показва се веднага след **реално** свързване, не и при обикновено
презареждане на страница с вече активна сесия:

- **Google Drive** — свързването минава през `signInWithOAuth` пренасочване
  (§9.1), т.е. страницата се презарежда изцяло и `driveConnected` тръгва от
  `false` при всяко зареждане, независимо дали е ново свързване или просто отворен
  таб с вече активна сесия. За да се различат двата случая, бутонът „Свържи с
  Google Drive“ пише `sessionStorage.setItem("pending_auto_upload_notice_drive", "1")`
  точно преди `driveConnect()`. Ефект следи `driveConnected` + `driveAutoSync`: ако
  `driveConnected === true` и флагът е налице, маха флага и — ако
  `driveAutoSync === "off"` — показва `autoUploadNotice = "drive"`.
- **Облак (Supabase)** — вход/регистрация (`connectWithEmail`) и „Активирай
  резервно копие в Облака“ (`enable`) стават без пренасочване, в рамките на същия
  рендер цикъл, затова не е нужен `sessionStorage`. `useRef` (`prevSupabaseEnabledRef`)
  пази предишната стойност на `supabaseEnabled`; ефект сравнява стар/нов `enabled`
  при всяка промяна и показва `autoUploadNotice = "supabase"` само при истински
  преход `false → true` (не и при mount с вече `enabled: true` от localStorage).

Причина да не се следи директно `driveConnected`/`supabaseEnabled` без флаг/`ref`:
и двете стойности стават `true` и при обикновено отваряне на приложението от вече
свързан потребител — модалът би изскачал при всяко зареждане, не само при
реално свързване.
...
### След тази сесия (2026-09-13)

- **83.** `App.jsx` — вграденият надпис „автоматичното качване е изключено“ в
  Drive/Облак секциите е заменен с модален прозорец (`autoUploadNotice`), който
  изисква изрично затваряне и се показва само при реално свързване (не и при
  презареждане на вече активна сесия) — `sessionStorage` флаг за Drive (заради
  OAuth пренасочването), `useRef` преход за Облака. Пълни подробности в §9.2.1.
